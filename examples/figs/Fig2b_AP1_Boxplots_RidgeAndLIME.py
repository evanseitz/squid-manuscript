...
avgFolder_lime = 'SQUID_13_AP1_intra_mut0_lime_k%s/pad%s' % (lime_k, fig_pad)
...
avgDir_lime = os.path.join(parentDir, 'examples_GOPHER/d_outputs_analysis/model_ResidualBind32_ReLU_single/%s' % avgFolder_lime)
...
avg_add_lime = pd.read_csv(os.path.join(avgDir_lime, 'ADD_A/ADD_%s/avg_additive_A.csv' % gauge), index_col=0)
...
tribox_lime = np.load(os.path.join(avgDir_lime, 'stats/stats_%s/compare_boxplot_A_values.npy' % gauge), allow_pickle='TRUE').item()
...
### Additive lime min
wtDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_lime, uniform_min)) #rank3_seq261
wt_add1_lime = pd.read_csv(os.path.join(wtDir, 'lime_k%s_additive.csv' % lime_k), index_col=0)
wt_add1_lime = squid_utils.fix_gauge(np.array(wt_add1_lime), gauge='hierarchical', wt=None)
wt_add1_lime = squid_utils.arr2pd(wt_add1_lime, alphabet)
### Additive lime max
wtDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_lime, uniform_max)) #rank15_seq2554
wt_add2_lime = pd.read_csv(os.path.join(wtDir, 'lime_k%s_additive.csv' % lime_k), index_col=0)
wt_add2_lime = squid_utils.fix_gauge(np.array(wt_add2_lime), gauge='hierarchical', wt=None)
wt_add2_lime = squid_utils.arr2pd(wt_add2_lime, alphabet)
...
add_boxes_lime = list(tribox_lime.values())[2]
...
print('add_lime lowest 10:', np.argsort(add_boxes_lime)[:15])
...
print('add_lime highest 10:', np.argsort(-1*add_boxes_lime)[:15])
...
for median in sal_plot['medians']:
    median.set_color('black')
add_plot_lime = ax1.boxplot(add_boxes_lime, sym='', widths=0.5, showfliers=False, showmeans=True, meanprops=flierprops,
                       positions=[C], vert=False)
for median in add_plot_lime['medians']:
    median.set_color('black')
...
singles_x = np.random.normal(C, 0.08, size=len(add_boxes_lime))
ax1.scatter(add_boxes_lime, singles_x, alpha=a, s=s, c=color_add_lime, zorder=-10)
if 0:
    ax1.scatter(add_boxes_lime[26], singles_x[26], s=s, c='blue', zorder=10)
    ax1.scatter(add_boxes_lime[14], singles_x[14], s=s, c='red', zorder=10)
...
set_box_color(add_plot_lime, color_add_lime)
...
if 0:
    ax1.set_yticklabels(['ISM', 'Saliency', 'LIME', 'Ridge', 'GE'], fontsize=12)
else:
...
if 1:
    ...
    mwu_stat, pval = stats.mannwhitneyu(add_boxes_NL, add_boxes_lime, alternative=alt)
    print('GE vs LIME: %s, %s' % (mwu_stat, pval))
...
if 1:
    ...
    logo = logomaker.Logo(df=wt_add1_lime[start-fig_pad:stop+fig_pad+1],
                    ax=ax4,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
    ...
    logo = logomaker.Logo(df=wt_add2_lime[start-fig_pad:stop+fig_pad+1],
                    ax=ax8,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
    ...
    logomaker.Logo(df=avg_add_lime,
                ax=ax12,
                fade_below=.5,
                shade_below=.5,
                width=.9,
