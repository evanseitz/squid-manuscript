...
avgFolder_lasso = 'SQUID_13_AP1_intra_mut0_lasso/pad%s' % fig_pad
avgFolder_ridge = 'SQUID_13_AP1_intra_mut0_ridgeCV/pad%s' % fig_pad

avgDir = os.path.join(parentDir, 'examples_GOPHER/d_outputs_analysis/model_ResidualBind32_ReLU_single/%s' % avgFolder)
avgDir_lasso = os.path.join(parentDir, 'examples_GOPHER/d_outputs_analysis/model_ResidualBind32_ReLU_single/%s' % avgFolder_lasso)
avgDir_ridge = os.path.join(parentDir, 'examples_GOPHER/d_outputs_analysis/model_ResidualBind32_ReLU_single/%s' % avgFolder_ridge)
...
avg_add_lasso = pd.read_csv(os.path.join(avgDir_lasso, 'ADD_A/ADD_%s/avg_additive_A.csv' % gauge), index_col=0)
avg_add_ridge = pd.read_csv(os.path.join(avgDir_ridge, 'ADD_A/ADD_%s/avg_additive_A.csv' % gauge), index_col=0)

tribox_NL = np.load(os.path.join(avgDir, 'stats/stats_%s/compare_boxplot_A_values.npy' % gauge), allow_pickle='TRUE').item()
tribox_lasso = np.load(os.path.join(avgDir_lasso, 'stats/stats_%s/compare_boxplot_A_values.npy' % gauge), allow_pickle='TRUE').item()
tribox_ridge = np.load(os.path.join(avgDir_ridge, 'stats/stats_%s/compare_boxplot_A_values.npy' % gauge), allow_pickle='TRUE').item()

wtFolder_NL = 'SQUID_13_AP1_intra_mut0'
wtFolder_ridge = wtFolder_NL#'SQUID_13_AP1_intra_mut0_ridgeCV'
wtFolder_lasso = wtFolder_NL
...
### Additive Lasso min
wtDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_lasso, uniform_min)) #rank3_seq261
wt_add1_lasso = pd.read_csv(os.path.join(wtDir, 'lasso_additive.csv'), index_col=0)
wt_add1_lasso = squid_utils.fix_gauge(np.array(wt_add1_lasso), gauge='hierarchical', wt=None)
wt_add1_lasso = squid_utils.arr2pd(wt_add1_lasso, alphabet)
### Additive Lasso max
wtDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_lasso, uniform_max)) #rank15_seq2554
wt_add2_lasso = pd.read_csv(os.path.join(wtDir, 'lasso_additive.csv'), index_col=0)
wt_add2_lasso = squid_utils.fix_gauge(np.array(wt_add2_lasso), gauge='hierarchical', wt=None)
wt_add2_lasso = squid_utils.arr2pd(wt_add2_lasso, alphabet)

### Additive Ridge min
wtDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_ridge, uniform_min)) #rank3_seq261
wt_add1_ridge = pd.read_csv(os.path.join(wtDir, 'ridge_additive.csv'), index_col=0)
wt_add1_ridge = squid_utils.fix_gauge(np.array(wt_add1_ridge), gauge='hierarchical', wt=None)
wt_add1_ridge = squid_utils.arr2pd(wt_add1_ridge, alphabet)
### Additive Ridge max
wtDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_ridge, uniform_max)) #rank15_seq2554
wt_add2_ridge = pd.read_csv(os.path.join(wtDir, 'ridge_additive.csv'), index_col=0)
wt_add2_ridge = squid_utils.fix_gauge(np.array(wt_add2_ridge), gauge='hierarchical', wt=None)
wt_add2_ridge = squid_utils.arr2pd(wt_add2_ridge, alphabet)
...
gs1 = GridSpec(5, 3, left=0.05, right=0.37, wspace=0.1, hspace=0.2)
ax1 = fig.add_subplot(gs1[:5, :3])
#ax1.axes.get_xaxis().set_ticks([])

gs0 = GridSpec(5, 3, left=0.38, right=0.88, wspace=0.1, hspace=0.2) #.38,.68
...
gs0 = GridSpec(5, 3, left=0.49+.055, right=0.99+.055, wspace=0.1, hspace=0.2) #.49, .79
...
ax15 = fig.add_subplot(gs0[4, 0])
ax15.axes.get_xaxis().set_ticks([])
ax15.axes.get_yaxis().set_ticks([])

gs0 = GridSpec(5, 3, left=0.60+.11, right=1.09+.11, wspace=0.1, hspace=0.2)
...
# Box plots
# =============================================================================
ism_boxes = list(tribox_ridge.values())[0]
sal_boxes = list(tribox_ridge.values())[1]
add_boxes_lasso = list(tribox_lasso.values())[2]
add_boxes_ridge = list(tribox_ridge.values())[2]
...
print('additive_ridge min/max index: %s, %s' % (np.argmin(add_boxes_ridge), np.argmax(add_boxes_ridge)))
...
print('add_lasso lowest 10:', np.argsort(add_boxes_lasso)[:15])
print('add_ridge lowest 10:', np.argsort(add_boxes_ridge)[:15])
...
print('add_lasso highest 10:', np.argsort(-1*add_boxes_lasso)[:15])
print('add_ridge highest 10:', np.argsort(-1*add_boxes_ridge)[:15])
...
for median in sal_plot['medians']:
    median.set_color('black')
add_plot_lasso = ax1.boxplot(add_boxes_lasso, sym='', widths=0.5, showfliers=False, showmeans=True, meanprops=flierprops,
                       positions=[C], vert=False)
for median in add_plot_lasso['medians']:
    median.set_color('black')
add_plot_ridge = ax1.boxplot(add_boxes_ridge, sym='', widths=0.5, showfliers=False, showmeans=True, meanprops=flierprops,
                       positions=[B], vert=False)
for median in add_plot_ridge['medians']:
    median.set_color('black')
...
color_add_lasso = '#999999' #gray
color_add_ridge = '#f781bf' #pink
color_add_NL = '#4daf4a' #green

ax1.set_xlabel('Error', fontsize=16, labelpad=3)

singles_x = np.random.normal(E, 0.08, size=len(ism_boxes))
ax1.scatter(ism_boxes, singles_x, alpha=a, s=s, c=color_ISM, zorder=-10)

singles_x = np.random.normal(D, 0.08, size=len(sal_boxes))
ax1.scatter(sal_boxes, singles_x, alpha=a, s=s, c=color_sal, zorder=-10)

singles_x = np.random.normal(C, 0.08, size=len(add_boxes_lasso))
ax1.scatter(add_boxes_lasso, singles_x, alpha=a, s=s, c=color_add_lasso, zorder=-10)
if 1:
    ax1.scatter(add_boxes_lasso[26], singles_x[26], s=s, c='blue', zorder=10)
    ax1.scatter(add_boxes_lasso[14], singles_x[14], s=s, c='red', zorder=10)


singles_x = np.random.normal(B, 0.08, size=len(add_boxes_ridge))
ax1.scatter(add_boxes_ridge, singles_x, alpha=a, s=s, c=color_add_ridge, zorder=-10)
...
set_box_color(add_plot_lasso, color_add_lasso)
set_box_color(add_plot_ridge, color_add_ridge)
...
if 0:
    ax1.set_yticklabels(['ISM', 'Saliency', 'Lasso', 'Ridge', 'GE'], fontsize=12)
else:
...
if 1:
    ...
    ax_tops.append([item.get_ydata()[1] for item in add_plot_ridge['caps']][1])
    ...
    ax_bots.append([item.get_ydata()[1] for item in add_plot_ridge['caps']][0])
    ...
    combinations = [(A, C), (A, B), (A, D), (A, E), (B, C), (B, D), (B, E), (C, D), (C, E)]
    ...
    print('GE vs ISM: %s, %s' % (mwu_stat, pval))

    mwu_stat, pval = stats.mannwhitneyu(add_boxes_NL, sal_boxes, alternative=alt)
    print('GE vs SAL: %s, %s' % (mwu_stat, pval))

    mwu_stat, pval = stats.mannwhitneyu(add_boxes_NL, add_boxes_lasso, alternative=alt)
    print('GE vs LASSO: %s, %s' % (mwu_stat, pval))

    mwu_stat, pval = stats.mannwhitneyu(add_boxes_NL, add_boxes_ridge, alternative=alt)
    print('GE vs RIDGE: %s, %s' % (mwu_stat, pval))
...
if 1:
    ...
    logo = logomaker.Logo(df=wt_add1_lasso[start-fig_pad:stop+fig_pad+1],
                    ax=ax4,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
    ...
    ref_arr = np.where(np.array(ref_attr)!=0., 1, 0)
    ref_seq = squid_utils.oh2seq(ref_arr[start-fig_pad:stop+fig_pad+1], ['A','C','G','T'])
    #logo.style_glyphs_in_sequence(sequence=ref_seq, color=color_add_ridge)

    logo = logomaker.Logo(df=wt_add1_ridge[start-fig_pad:stop+fig_pad+1],
                    ax=ax5,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
    ...
    logo = logomaker.Logo(df=wt_add2_lasso[start-fig_pad:stop+fig_pad+1],
                    ax=ax8,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
    ...
    refDir = os.path.join(parentDir, 'examples_GOPHER/c_surrogate_outputs/model_ResidualBind32_ReLU_single/%s/%s' % (wtFolder_ridge, uniform_max)) #rank15_seq2554
    ref_attr = pd.read_csv(os.path.join(refDir, 'attributions_ISM_single.csv'), index_col=0)
    ref_arr = np.where(np.array(ref_attr)!=0., 1, 0)
    ref_seq = squid_utils.oh2seq(ref_arr[start-fig_pad:stop+fig_pad+1], ['A','C','G','T'])
    #logo.style_glyphs_in_sequence(sequence=ref_seq, color=color_add_ridge)

    logo = logomaker.Logo(df=wt_add2_ridge[start-fig_pad:stop+fig_pad+1],
                    ax=ax9,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
    ...
    logomaker.Logo(df=avg_add_lasso,
                ax=ax12,
                fade_below=.5,
                shade_below=.5,
                width=.9,
    ...
    logomaker.Logo(df=avg_add_ridge,
                    ax=ax13,
                    fade_below=.5,
                    shade_below=.5,
                    width=.9,
